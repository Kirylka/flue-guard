...
| `GuardDeniedError` | `guard_denied` | The guard denied the action. |
| `GuardUnavailableError` | `guard_unavailable` | Guard evaluation, argument cloning, cancellation, or its deadline failed. Execution is blocked. |
...
`GovernanceErrorCode` is the union of the codes above.
...
try {
  ...
  if (isApprovalPending(err)) {
    ...
    // Configuration error or unavailable guard; execution was blocked.
    throw err;
  } else {
...
- `isGovernanceDenial(err)` matches refusals only: `missing_context`,
  ...
  `guard_denied`, `approval_denied`, `idempotency_conflict`. It excludes `approval_pending`
  (not a refusal), `guard_unavailable` (assessment failed), and `config_error`
